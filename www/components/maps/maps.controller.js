(function(){
  ...
  function MapsController($state, $log, $http, $scope, $ionicModal, $ionicPopup, $ionicScrollDelegate, localStorageService, urlFactory, currentUser){
    ...
    $scope.closeDetailModal = function() {
      $ionicScrollDelegate.$getByHandle('mapZoom').zoomTo(1, false)
    ...
    function onDoubleTap(){
      if (zoom === 0) {
        zoom++
        $ionicScrollDelegate.$getByHandle('mapZoom').zoomTo(2, true)
      } else {
        zoom--
        $ionicScrollDelegate.$getByHandle('mapZoom').zoomTo(1, true)
      }
    }
